...
# generate data
W_true = np.random.randn(N,R) # generate univariate gaussian noise with shape NxR
C_true = np.random.randn(R,N) #true values of C 

Y_true = np.dot(W_true, C_true) #true 
Y_tf = tf.constant(Y_true.astype(np.float32))

""" 
what's the diff between w and c?  
"""
